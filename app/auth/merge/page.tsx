...
function MergeContent() {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      if (!guestId) { router.replace(next); return }
      ...
      if (!basket) { router.replace(next); return }
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full p-6">
        ...
        <div className="flex gap-3">
          <button onClick={() => handleMerge(true)} disabled={loading} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded disabled:opacity-50">Yes, merge</button>
          <button onClick={() => handleMerge(false)} disabled={loading} className="flex-1 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-300 py-2 rounded">No, discard</button>
        </div>
      </div>
    </div>
  )
