...
export default function ProductForm() {
  ...
  // Auto‑detect sex based on name + description
  useEffect(() => {
    const text = (name + ' ' + description).toLowerCase()
    const femaleKeywords = ['female', 'women', 'woman', 'girl', 'ladies', 'dress']
    if (femaleKeywords.some(keyword => text.includes(keyword))) {
      setSex('women')
    } else {
      setSex('men')
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Require at least one media (image or video)
  ...
  return (
    <>
    <form onSubmit={handleSubmit} className="bg-white dark:bg-[#1e1e1e] p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 max-w-2xl mx-auto">
      ...
      <div className="space-y-4">
        <input
          ...
          className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none"
        />
    ...
    Browse Catalog
        ...
        <div className="grid grid-cols-2 gap-4">
          <input
            ...
            placeholder="Price *"
            required
            className="border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none"
          />
          <input
            name="stock"
            type="number"
        ...
        {/* Image Upload - Optional now (but at least one media required) */}
        <div className="relative border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 hover:border-blue-500 transition">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              <span className="font-semibold text-blue-600 dark:text-blue-400">Click to upload image</span> (optional, but at least one media required)
            </p>
        ...
        {/* Video Upload - Optional (but at least one media required) */}
        <div className="relative border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 hover:border-purple-500 transition">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              <span className="font-semibold text-purple-600 dark:text-purple-400">Click to upload video</span> (optional, but at least one media required)
            </p>
